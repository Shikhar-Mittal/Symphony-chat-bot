...
public class AIDataService {
	...
	/**
	 * Create new service for given configuration and some predefined service
	 * context
	 * 
	 * @param config
	 *            Service configuration data. Cannot be <code>null</code>
	 * @param serviceContext
	 *            Service context. If <code>null</code> then new context will be
	 *            created
	 * @throws IllegalArgumentException
	 *             If config parameter is null
	 */
	 ...
	 *
	 * @param request
	 *            request object to the service. Cannot be <code>null</code>
	 ...
	 *            request object to the service. Cannot be <code>null</code>
	 * @param serviceContext
	 *            custom service context that should be used instead of the default
	 *            context
	 * @return response object from service. Never <code>null</code>
	 */
	 ...
	 *            request object to the service. Cannot be <code>null</code>
	 * @param requestExtras
	 *            object that can hold additional contexts and entities
	 * @return response object from service. Never <code>null</code>
	 */
	...
	public AIResponse request(final AIRequest request, final RequestExtras requestExtras,
		...
		try {
			...
			final String response = doTextRequest(config.getQuestionUrl(request.getSessionId()), queryData,
					additionalHeaders);
			...
			System.out.println("Response=" + aiResponse.getResult().getSource());
			if (aiResponse == null) {
			...
			if (aiResponse.isError()) {
				System.out.println("We got a error:");
	 ...
	 *
	 * @param voiceStream
	 *            voice data stream for recognition. Cannot be <code>null</code>
	 * @return response object from service. Never <code>null</code>
	 * @throws AIServiceException
	 */
	 ...
	 *            voice data stream for recognition. Cannot be <code>null</code>
	 * @param aiContexts
	 *            additional contexts for request
	 * @return response object from service. Never <code>null</code>
	 * @throws AIServiceException
	 */
	...
	public boolean resetContexts() {
		...
		cleanRequest.setQuery("empty_query_for_resetting_contexts"); // TODO
																		// remove
																		// it
																		// after
																		// protocol
	 ...
	 * @throws AIServiceException
	 */
	public List<AIContext> getActiveContexts(final AIServiceContext serviceContext) throws AIServiceException {
		try {
			return doRequest(ApiActiveContextListResponse.class, config.getContextsUrl(getSessionId(serviceContext)),
					REQUEST_METHOD_GET);
	...
	public AIContext getActiveContext(final String contextName, final AIServiceContext serviceContext)
			throws AIServiceException {
		try {
			return doRequest(AIContext.class, config.getContextsUrl(getSessionId(serviceContext), contextName),
					REQUEST_METHOD_GET);
		} catch (BadResponseStatusException e) {
	 ...
	 * 
	 * @param contexts
	 *            Iterable collection of contexts
	 * @return List of added context names, or empty list if no contexts were added
	 * @throws AIServiceException
	 */
	public List<String> addActiveContext(final Iterable<AIContext> contexts) throws AIServiceException {
	...
	public void resetActiveContexts(final AIServiceContext serviceContext) throws AIServiceException {
		try {
			doRequest(AIResponse.class, config.getContextsUrl(getSessionId(serviceContext)), REQUEST_METHOD_DELETE);
		} catch (BadResponseStatusException e) {
			throw new AIServiceException(e.response);
		}
	}
	 ...
	 * @param contextName
	 *            The context name
	 * @return <code>false</code> if context was not delete, <code>true</code> in
	 *         otherwise case
	 * @throws AIServiceException
	 */
	 ...
	 * 
	 * @param userEntities
	 *            collection of a new entity data
	 * @return response object from service. Never <code>null</code>
	 * @throws AIServiceException
	 */
	 ...
	 * @throws AIServiceException
	 */
	public AIResponse uploadUserEntities(final Collection<Entity> userEntities, AIServiceContext serviceContext)
			throws AIServiceException {
		...
		try {
			final String response = doTextRequest(config.getUserEntitiesEndpoint(getSessionId(serviceContext)),
					requestData);
			if (StringUtils.isEmpty(response)) {
				throw new AIServiceException(
						"Empty response from ai service. Please check configuration and Internet connection.");
	...
	protected String doTextRequest(final String requestJson, AIServiceContext serviceContext)
			throws MalformedURLException, AIServiceException, KeyStoreException, NoSuchAlgorithmException,
	...
	protected String doTextRequest(final String requestJson)
			throws MalformedURLException, AIServiceException, KeyStoreException, NoSuchAlgorithmException, Exception {
	...
	protected String doTextRequest(final String endpoint, final String requestJson,
		...
		try {
			...
			// logger.error(
			// "Can't make request to the API.AI service. Please, check connection settings
...
//					throw new AIServiceException(response);
			...
			// e);
			throw new AIServiceException(
					"Can't make request to the API.AI service. Please, check connection settings and API.AI keys.", e);
		} finally {
			if (connection != null) {
				connection.disconnect();
			}
	...
	protected <TRequest, TResponse> TResponse doRequest(final TRequest request, final Type responseType,
			final String endpoint, final String method) throws AIServiceException, BadResponseStatusException {
		return doRequest(request, responseType, endpoint, method, (Map<String, String>) null);
	}

	protected <TResponse> TResponse doRequest(final Type responseType, final String endpoint, final String method,
			final Map<String, String> additionalHeaders) throws AIServiceException, BadResponseStatusException {
		return doRequest((Object) null, responseType, endpoint, method, additionalHeaders);
	}

	protected <TRequest, TResponse> TResponse doRequest(final TRequest request, final Type responseType,
			final String endpoint, final String method, final Map<String, String> additionalHeaders)
			throws AIServiceException, BadResponseStatusException {
	...
	private String getTimeZone(AIServiceContext serviceContext) {
		TimeZone timeZone = serviceContext != null ? serviceContext.getTimeZone() : defaultServiceContext.getTimeZone();
